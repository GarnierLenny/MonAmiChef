...
export class MacroCalculationService {
   ...
   */
  static async calculateTodayMacros(userId: string): Promise<MacroNutrition> {
   ...
   */
  static async calculateWeekMacros(userId: string): Promise<DayMacros[]> {
    const today = new Date();
    const weekStart = this.getWeekStart(today);
   ...
   */
  private static parseNutrition(nutritionJson: any): MacroNutrition | null {
    ...
    try {
      ...
      const protein = this.extractNutrientValue(nutrition, [
        "protein", "proteins", "protein_g", "proteinGrams"
      ]);

      const carbs = this.extractNutrientValue(nutrition, [
        "carbs", "carbohydrates", "carbs_g", "carbsGrams", "carbohydrate"
      ]);

      const fat = this.extractNutrientValue(nutrition, [
        "fat", "fats", "fat_g", "fatGrams", "totalFat"
      ]);

      const calories = this.extractNutrientValue(nutrition, [
        "calories", "kcal", "energy", "calorie", "totalCalories"
      ]);
   ...
   */
  private static extractNutrientValue(nutrition: any, possibleKeys: string[]): number {
