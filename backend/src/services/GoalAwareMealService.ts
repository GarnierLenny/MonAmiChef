import { MacroCalculationService, type MacroNutrition } from './MacroCalculationService';
  ...
  async getGoalContext(userId: string, day: string): Promise<GoalContext> {
    try {
      ...
      if (goalParts.length > 0) {
        ...
        goalMessage += "SERVING SIZE CONSTRAINT: Always set servings = 1. To reach the calorie target, use calorie-dense ingredients (nuts, oils, dairy, proteins), larger portions, and multiple dishes. Examples: avocado toast + protein smoothie + fruit, or salmon + quinoa + vegetables with olive oil. The meal should be filling and substantial for ONE person.";
      }
