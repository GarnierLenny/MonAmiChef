...
const prompts: ExamplePrompt[] = [
  { icon: '⚡', text: 'Quick dinner for 2', category: 'fast' },
  { icon: '🥗', text: 'Healthy vegetarian recipes', category: 'healthy' },
  { icon: '🍗', text: 'What can I make with chicken?', category: 'ingredient' },
  { icon: '🍝', text: 'Easy pasta dishes', category: 'easy' },
  { icon: '🌶️', text: 'Spicy Asian cuisine', category: 'cuisine' },
  { icon: '🥘', text: 'Family meal ideas', category: 'family' },
];
...
const categoryColors: Record<ExamplePrompt['category'], string> = {
  ...
  healthy: 'from-green-100 to-green-50 hover:from-green-200 hover:to-green-100 border-green-200',
  ingredient: 'from-yellow-100 to-yellow-50 hover:from-yellow-200 hover:to-yellow-100 border-yellow-200',
  easy: 'from-blue-100 to-blue-50 hover:from-blue-200 hover:to-blue-100 border-blue-200',
  cuisine: 'from-red-100 to-red-50 hover:from-red-200 hover:to-red-100 border-red-200',
  family: 'from-purple-100 to-purple-50 hover:from-purple-200 hover:to-purple-100 border-purple-200',
};

export default function ExamplePrompts({ onPromptClick, mini = false }: ExamplePromptsProps) {
  ...
  return (
    <div className={`${mini ? 'space-y-2' : 'space-y-3'}`}>
      ...
      <div className={`${mini ? 'space-y-2' : ''}`}>
        ...
        <div className={`md:hidden ${mini ? '' : 'overflow-x-auto pb-2 -mx-4 px-4 no-scrollbar'}`}>
          <div className={`flex ${mini ? 'flex-col space-y-2' : 'gap-2.5 min-w-max'}`}>
        ...
        {/* Desktop: 2x2 Grid */}
        <div className={`hidden md:grid ${mini ? 'grid-cols-1 gap-2' : 'grid-cols-2 gap-2.5 max-w-3xl mx-auto'}`}>
          {displayPrompts.map((prompt, index) => (
            <PromptButton
              key={prompt.text}
              prompt={prompt}
  ...
  return (
    ...
    >
      <div className="flex items-center gap-2.5">
        <span className={`${mini ? 'text-xl' : 'text-xl md:text-2xl'} group-hover:scale-110 transition-transform duration-200 flex-shrink-0`}>
          {prompt.icon}
        </span>
